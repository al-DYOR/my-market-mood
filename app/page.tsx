...
const CONFIG = {
  ...
  SKIN_REQUIRED: BigInt("4164305") * BigInt("1000000000000000000"),
  BYEMONEY_TOKEN: "0xa12a532b0b7024b1d01ae66a3b8cf77366c7db07",
  BYEMONEY_REQUIRED: BigInt("1000000") * BigInt("1000000000000000000"),
  BYEMONEY_RECEIVER: "0x063c4989cFb40557353EDB8c26Ea8009789F2569",
  NFT_CONTRACT: "0x739aF547F2468915dA73fCB5fF66f07E4f632c32",
}
...
function generateUniqueName(
  ...
  const orderedCalmLight = [
    "Horizon", "Serenity", "Crystal", "Haven", "Clarity", "Oasis", "Dawn", "Zenith", "Pearl", "Lotus",
    "Prism", "Aurora", "Tranquil", "Serene", "Placid", "Harmony",
  ]
  const orderedCalmDark = [
    "Depths", "Eclipse", "Crypt", "Silence", "Vault", "Abyss", "Midnight", "Sanctum", "Obsidian", "Onyx",
    "Raven", "Nocturne", "Shadow", "Void", "Somber", "Grave",
  ]
  const orderedAnxiousLight = [
    "Flare", "Spark", "Prism", "Surge", "Blaze", "Radiance", "Ignite", "Pulse", "Flicker", "Shimmer",
    "Dazzle", "Glint", "Luminous", "Vibrant", "Bright", "Flash",
  ]
  const orderedAnxiousDark = [
    "Ember", "Shadow", "Specter", "Wraith", "Vortex", "Phantom", "Obsidian", "Torment", "Dire", "Grim",
    "Dread", "Haunt", "Sinister", "Omen", "Gloom", "Menace",
  ]
  const chaoticCalmLight = [
    "Drift", "Breeze", "Mist", "Float", "Whisper", "Wander", "Shimmer", "Ripple", "Gentle", "Soft",
    "Wisp", "Murmur", "Lull", "Soothe", "Calm", "Peace",
  ]
  const chaoticCalmDark = [
    "Haze", "Gloom", "Shroud", "Dusk", "Murk", "Twilight", "Shade", "Mire", "Dim", "Fog",
    "Blur", "Smog", "Overcast", "Gray", "Dusky", "Misty",
  ]
  const chaoticAnxiousLight = [
    "Storm", "Flash", "Burst", "Shatter", "Crackle", "Lightning", "Thunder", "Tempest", "Chaos", "Wild",
    "Fierce", "Rage", "Fury", "Blast", "Crash", "Strike",
  ]
  const chaoticAnxiousDark = [
    "Chaos", "Void", "Rupture", "Ruin", "Inferno", "Maelstrom", "Apocalypse", "Cataclysm", "Carnage", "Havoc",
    "Mayhem", "Turmoil", "Bedlam", "Frenzy", "Pandemonium", "Anarchy",
  ]
  const focusedWords = [
    "Anchor", "Focus", "Beacon", "Core", "Truth", "Pillar", "Foundation", "Compass", "Center", "Balance",
    "Steady", "Grounded", "Stable", "Fixed", "Firm", "Solid",
  ]
  const distractedWords = [
    "Scatter", "Ripple", "Fragment", "Flux", "Wander", "Nebula", "Cascade", "Divide", "Drift", "Disperse",
    "Diverge", "Split", "Branch", "Spread", "Diffuse", "Stray",
  ]
  const fallbackWords = [
    "Essence", "Nexus", "Ethos", "Cipher", "Omen", "Enigma", "Relic", "Primal", "Vestige", "Genesis",
    "Axiom", "Synthesis", "Quantum", "Catalyst", "Paradox", "Vertex", "Zenon", "Apex", "Epoch", "Matrix",
    "Spiral", "Helix", "Cortex", "Nexus", "Zephyr", "Vortex", "Cipher", "Oracle", "Astral", "Cosmic",
    "Stellar", "Lunar", "Solar", "Nova", "Nebula", "Comet", "Meteor", "Galaxy", "Pulsar", "Quasar",
    "Photon", "Proton", "Neutron", "Electron", "Atom", "Molecule", "Particle", "Wave", "Field", "Force",
    "Vector", "Scalar", "Tensor", "Fractal", "Mandala", "Sigil", "Rune", "Glyph", "Symbol", "Emblem",
  ]
  const extendedPool = [
    "Praxis", "Logos", "Mythos", "Cosmos", "Chronos", "Kairos", "Ethos", "Pathos", "Telos", "Arete",
    "Psyche", "Soma", "Pneuma", "Karma", "Dharma", "Nirvana", "Samsara", "Moksha", "Atman", "Brahman",
    "Tao", "Qi", "Yin", "Yang", "Zen", "Koan", "Satori", "Bodhi", "Metta", "Karuna",
    "Aura", "Chakra", "Mantra", "Yantra", "Tantra", "Mudra", "Asana", "Prana", "Kundalini", "Shakti",
    "Shiva", "Vishnu", "Brahma", "Maya", "Lila", "Leela", "Rasa", "Bhava", "Ananda", "Sat",
  ]
...
function getUsedNames(walletAddress: string, farcasterFid?: string): string[] {
...
function saveUsedName(walletAddress: string, name: string, farcasterFid?: string) {
  if (typeof window === "undefined") return
  try {
    const userKey = farcasterFid ? `${walletAddress.toLowerCase()}_${farcasterFid}` : walletAddress.toLowerCase()
    const key = `usedNames_${userKey}`
...
export default function Home() {
  ...
  return
      ...
      setByemoneyRequired(CONFIG.BYEMONEY_REQUIRED)
    } catch (error) {
...
const burnTxHash = await (async () => {
  ...
  return
      ...
      console.log("Calling mint on contract...")
  ...
  throw new Error("No wallet available for mint")
      ...
      // Ожидание receipt (опционально, можно оставить для извлечения tokenId)
      let receipt = null
      let attempts = 0
      while (!receipt && attempts < 30) {
        await new Promise((resolve) => setTimeout(resolve, 2000))
        receipt = await publicClient.getTransactionReceipt({ hash: mintHash })
        attempts++
      }
